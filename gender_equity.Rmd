...
output_edu_home_cb <- census_microdata081122 %>%
...
inc_breakdown_male_single <- lville_2019 %>% 
...
multi_eaner_hh_breakdown <- lville_2019 %>% 
...
#Old code don't need
```{r, eval=FALSE}
#Find's single male/female ownership rate 
single_own <- lville_2019 %>% 
  filter(earner_type == 'single_earner' & str_detect(hh_type,"own")) %>%
  group_by(sex) %>%
  summarize(n=n()) %>% 
  mutate(total = sum(n), 
         owner_rate = n/total*100
  )
```
