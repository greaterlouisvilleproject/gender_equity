...
As the ranking graphs show, Louisville ranks 2nd in homeownership among women without children, but 16th among women with children. 
...
write_csv(pull_peers(H_earntype_kids, add_info = TRUE), "earntype_slopegraph.csv")
...
E_singM_singF_plot <- ggplot(E_singM_singF, 
       aes(x=sex, 
           ...
           fill = educ)) + 
  geom_bar(stat="identity", position = "fill") +
  geom_text(aes(y = rate, label = percent(rate, scale = 1, accuracy = 0.1)), position = position_fill(vjust = 0.5), size = 24)
...
E_singM_singF <- census_microdata081122_person %>% 
  ...
  mutate(
    ...
    educ = factor(educ, 
                  levels = rev(c("no_hs", "hs", "some_col", "assoc", "bach","grad")), 
                  ordered = TRUE),
    kd_pres = factor(kd_pres, levels = c("kids", "no_kids"), labels = c("With Children", "Without Children"))) %>%
  ungroup()
...
E_singM_singF_plot <- E_singM_singF_plot + 
  facet_wrap(~kd_pres) +
  geom_text(aes(y = rate, label = percent(rate, accuracy = 0.1)), position = position_fill(vjust = 0.5), size = 24)
