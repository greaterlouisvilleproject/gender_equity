...
#income brackets based on HHINCOME
  ...
  kd_pres = if_else(any(age < 18), "kids", "no_kids"),
...
png("test1.png")
ggplot(demog_data, aes(x = row_num, y = col_num, color = gender)) + 
  geom_point(size = 10) 
dev.off()

png("test2.png")
ggplot(demog_data, aes(x = row_num, y = col_num, color = earn_type)) + 
  geom_point(size = 10) 
dev.off()
     ...
     scrolly_container(
       "scr",
        scrolly_graph(
          br(),
          br(),
          div(imageOutput("main_image"))),
       scrolly_sections(
         scrolly_section(
           id = 1,
           div(HTML("<H2> No education credentials </H2>
        ...
        <br><br> There are 23,765,700 workers with <font color='#A00042'>no formal education credential</font>.<p>"))),
         scrolly_section(
           id = 2,
           div(HTML("<H2> High school diplomas </H2>
        ...
        <br><br> There are 33,129,910 workers with a <font color='#F56C42'>high school diploma</font>.<p>"))),
         scrolly_section(id = 3,
           div(br()))))))

# Define server logic required to draw a histogram
server <- function(input, output) {

  roster.rx <- reactiveValues(scr = NULL)
  ...
  output$main_image <- renderImage({
    ...
    image_name <- paste0("test", add, ".png")
    img(src = "www/test1.png",
...
## Income Compared to Living Wage {.tabset}
...
```{r, eval = FALSE}
m <- trend_data_maxmin(I_CB_earn_trend, "single_fem_earner")
...
```
## Cost Burdened by Age {.tabset}

### All
```{r Cost Burdened Status by Age, eval=FALSE}
...
### Multiple Earner - none exist apparently
```{r, eval=FALSE}
#use command below for verification
#unique(cost_burden_age_sf$earner_type_d)
...
## Single Earner Female Homeownership Trend {.tabset}
...
### Ranking without Children - needs to be fixed to include children...ask harrison
...
## Homeownership by Race and Children {.tabset}
