...
showtext_auto()
font_add("Museo Sans", "/Users/joshmarkwell/Desktop/Professional/GLP/MuseoSans_300.otf")

```


```{r Fucntions, include=FALSE}

#Function used to compare groups rather than same group over time
...
##change to GLP colors
##use the same colors for gender throughout (blue/orange)
## create hit for each of the most three recent years -> just for verification...that there isnt anything weird
...
#re-do with function from glptools
  
p<-survey_by_demog(census_microdata081122, weight_var = "HHWT", 'homeownership') %>%
  filter(var_type == 'percent',
         race == 'total',
         sex != 'total')

trend(p, homeownership, rollmean = 3, plot_title = "Homeownership by Year", cat = 'sex', y_title = 'Percent',
       caption_text = "Source Greater Louisville Project
      Data from GLP analysis of ACS microdata from IPUMS-USA"
     )

# ggplot(p, aes(x=year, y=homeownership, group=sex)) +
...
#try and make a trend line like the chunk above
a <- census_microdata081122 %>% 
  filter(sex == 'female') %>%
  select(-sex) %>%
    mutate(homeownership = replace(homeownership, homeownership == T, 'Homeowner'),
         homeownership = replace(homeownership, homeownership == F, 'Renter')) 
  
trend(a, homeownership, rollmean = 3, plot_title = "Homeownership by Year", cat = 'earner_type')
  
# ggplot( aes(fill=earner_type, y=HHWT, x=homeownership)) + 
#     geom_bar(position="stack", stat="identity")

# ggplot( aes(fill=earner_type, y=HHWT, x=homeownership, weights = HHWT)) + 
...
# geom_bar(stat="identity", position=position_dodge())
  ...
  # ggplot( aes(fill=race, y=HHWT, x=homeownership, weights = HHWT)) + 
  #   geom_bar(position="stack", stat="identity")
...
ggplot( aes(x=race, y=HHWT, fill=educ, weights = HHWT)) + 
