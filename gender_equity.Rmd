...
save(H_gen_trend, H_singFem_rank, H_sinFem_kids, H_s_Femkids_trend, H_Fem_race, I_CB_earn_trend, cost_burden_age_sf, lville_2019,
...
E_singM_singF_plot <- E_singM_singF_plot + 
  ...
  labs(
  title = "Single Female vs. Single Male Education Breakdown",
) + 
  ylab(" ") +
  xlab(" ") +
...
cost_burden_age_sf_facet_plt <- ggplot(cost_burden_age_sf,
       aes(x=age_group, y=HHWT , fill=cost_burden),
        color="#00A9B7") +
geom_bar(stat="identity", position='fill')+
...
cost_burden_age_sf_facet_plt <- cost_burden_age_sf_facet_plt + 
  ...
  labs(
  title = "Cost Burdened Status by Earner Type",
) + 
  ylab(" ") +
  xlab(" ") +
  scale_fill_discrete(labels = c("Non Cost Burdened", "Cost Burdened")) + 
