...
homeownership_age_sf <- census_microdata081122 %>% 
    filter(year %in% 2017:2019) %>%
  mutate(
    cost_burden = factor(homeownership, 
                         levels = rev(c(TRUE, FALSE)), 
...
earntype_race_cb <- census_microdata081122 %>%
  group_by(FIPS, year, race, earner_type_d) %>%
  summarize(cost_burden = sum(HHWT[cost_burden]) / sum(HHWT) * 100, .groups = "drop")

earntype_cb  <- census_microdata081122 %>%
  group_by(FIPS, year, earner_type_d) %>%
  summarize(cost_burden = sum(HHWT[cost_burden]) / sum(HHWT) * 100, .groups = "drop") %>%
  mutate(race = "total")

earntype_cb %<>%
  bind_rows(earntype_race_cb) %>%
...
H_earntype_kids_cost_burden <- census_microdata081122 %>%
  filter(earner_type == 'single_earner') %>%
  survey_by_demog( weight_var = "HHWT", 'cost_burden', other_grouping_vars = c("kd_pres"))
...
gender_earnings_labels <- data.frame (ind_code  = c("11","21","22","23","31","32","33","42","44","45","48","49", "51","52","53","54","55","56","61","62","71","72","81","92"),
                  
                    description = c("Agriculture,Forestry, Fishing and Hunting", 
...
save(H_gender, H_earntype, H_earntype_kids, CB_earntype, gender_earnings_labels, H_earntype_kids_cost_burden,
     earntype_cb,
...
```{r, include = FALSE}
View(glpdata::industry_gender_earnings)

ind_gender_earn_temp <- glpdata::industry_gender_earnings

ind_gender_earn_temp$ind_code = substr(ind_gender_earn_temp$industry,1,2)
ind_gender_earn_temp <- ind_gender_earn_temp %>% relocate(ind_code, .before = FIPS)

industry_gender_final <- merge(ind_gender_earn_temp, gender_earnings_labels, by = "ind_code")

industry_gender_final <- industry_gender_final %>% 
        rename(description_broad = description)

```

```{r, include = FALSE}

sf_ind_2019 <- industry_gender_final %>%
  filter(FIPS == 21111,
         year == 2019,
         ind_level == 2) %>% #keeps from double or triple counting
  group_by(sex, description_broad) %>%
  summarize(n = sum(jobs)
            )%>%
  mutate(total = sum(n),
         rate = n/sum(n) *100
  )

female_sf_ind_2019 <- sf_ind_2019 %>%
  filter(sex == "female")
##Top three sectors:
#Health Care & Social Assistance, Retail, Accomodation & Food Services

male_sf_ind_2019 <- sf_ind_2019 %>%
  filter(sex == "male") 
##Top three sectors:
#Manufacturing, Transportation & Warehousing, Retail
  ...
  filter(
   ...
   ggplot( aes(x=HHINCOME, 
    ...
    geom_histogram(alpha=0.5, position = 'identity', binwidth = 10000) +
     ...
     annotate("text", x=living_wage - 3500, y=0.3, label=dollar(living_wage, 
                                                                       scale = 0.001,
                                                                      accuracy = 0.1,
                                                                       suffix = "k"), 
                                                                          angle=0,
