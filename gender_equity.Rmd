...
other_female_earner
```
interesting to see the faceted histogram tabset for 0,1,2 and 3 kids. 
...
```{r}
func_income_by_kids <- function(kids) {
...
### Single Female Earner with Children
```{r}

#why is color not working?
#still need to add living wage lines

kids_female_earner <- func_income_by_kids("kids")
kids_female_earner <- kids_female_earner + 
  labs(
  title = "Female Single Earner Income, with Children",
) + 
...
### Single Female Earner without Children
```{r}
no_kids_female_earner <- func_income_by_kids("kids")
no_kids_female_earner <- no_kids_female_earner + 
  labs(
  title = "Female Single Earner Income, without Children",
) + 
...
#incomplete
#need help with y axis...making a percent...there also seems to be two values per income bin
#could not figure out labeling issue for x axis
#push to gh for harrison...ha

these_labels <- paste0(
  dollar(seq(1, 273500, 10000),
         scale = 0.001, accuracy = 1, suffix = "k"),
  "-",
  dollar(seq(10001, 283500, 10000),
         scale = 0.001, accuracy = 1, suffix = "k"))

these_labels2 <- paste0(dollar(seq(1, 273500, 10000), scale = 0.001, accuracy = 1, suffix = "k"))
