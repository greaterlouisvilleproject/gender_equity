...
H_earntype_race <- census_microdata081122 %>%
  group_by(FIPS, year, race, male_fem_mult_earn) %>%
  summarize(homeownership = sum(HHWT[homeownership]) / sum(HHWT) * 100, .groups = "drop")

H_earntype  <- census_microdata081122 %>%
  group_by(FIPS, year, male_fem_mult_earn) %>%
  summarize(homeownership = sum(HHWT[homeownership]) / sum(HHWT) * 100, .groups = "drop") %>%
  mutate(race = "total")

H_earntype %<>%
  bind_rows(H_earntype_race) %>%
  select(FIPS, year, race, male_fem_mult_earn, homeownership)
...
H_earntype <- survey_by_demog(census_microdata081122, 
                              weight_var = "HHWT", 
                              'homeownership', 
                              other_grouping_vars = c('male_fem_mult_earn'))
